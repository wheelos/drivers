...
#include "lidar/robosense/decoder/decoder_base.hpp"
namespace apollo {
namespace drivers {
namespace robosense {
...
} __attribute__((packed)) RSBP_MsopBlock;
...
typedef struct {
  ...
  uint32_t index;
  uint16_t tail;
} __attribute__((packed)) RSBP_MsopPkt;

typedef struct {
  uint8_t reserved[240];
  uint8_t coef;
  uint8_t ver;
} __attribute__((packed)) RSBP_Intensity;
...
typedef struct {
  ...
  uint16_t tail;
} __attribute__((packed)) RSBP_DifopPkt;

template <typename vpoint>
class DecoderBP : public DecoderBase<vpoint> {
 public:
  explicit DecoderBP(const RS_Param &param);
...
template <typename vpoint>
DecoderBP<vpoint>::DecoderBP(const RS_Param &param)
...
double DecoderBP<vpoint>::getLidarTime(const uint8_t *pkt) {
  RSBP_MsopPkt *mpkt_ptr =
      const_cast<RSBP_MsopPkt *>(reinterpret_cast<const RSBP_MsopPkt *>(pkt));
  std::tm stm;
...
template <typename vpoint>
int DecoderBP<vpoint>::decodeMsopPkt(
  ...
  *height_ptr = 32;
  ...
  for (int blk_idx = 0; blk_idx < RSBP_BLOCKS_PER_PKT; blk_idx++) {
    ...
    if (this->echo_mode_ == RS_ECHO_DUAL) {
      if (blk_idx < (RSBP_BLOCKS_PER_PKT - 2)) {
      ...
      if (blk_idx < (RSBP_BLOCKS_PER_PKT - 1)) {
    ...
    for (int channel_idx = 0; channel_idx < RSBP_CHANNELS_PER_BLOCK;
         channel_idx++) {
      int azimuth_final;

      azimuth_channel =
          azimuth_blk + (azimuth_diff * RSBP_CHANNEL_TOFFSET *
                         (channel_idx % 16) / RSBP_FIRING_TDURATION);
...
int32_t DecoderBP<vpoint>::decodeDifopPkt(const uint8_t *pkt) {
  RSBP_DifopPkt *rsbp_ptr =
      const_cast<RSBP_DifopPkt *>(reinterpret_cast<const RSBP_DifopPkt *>(pkt));

  if (rsbp_ptr->sync != RSBP_DIFOP_SYNC) {
    return -2;
  }

  RS_Version *p_ver = &(rsbp_ptr->version);
  ...
  int pkt_rate =
      ceil(RSBP_POINTS_CHANNEL_PER_SECOND / RSBP_BLOCKS_CHANNEL_PER_PKT);
  ...
  if (!(this->cali_data_flag_ & 0x2)) {
    ...
    p_pitch_cali = rsbp_ptr->pitch_cali;
    ...
    if (angle_flag) {
      ...
      const uint8_t *p_yaw_cali = rsbp_ptr->yaw_cali;
      for (int i = 0; i < 32; i++) {
        ...
        this->vert_angle_list_[i] =
            (mid * 256 + msb) * neg * 0.01f;  // / 180 * M_PI;
        lsb = p_yaw_cali[i * 3];
        if (lsb == 0) {
          neg = 1;
        } else if (lsb == 1) {
      ...
      // std::cout << "[RS_decoder][difop][INFO] angle data is wrote in difop
      // packet!" << std::endl;
    }
  }
